...
if ($action && $attemptid && confirm_sesskey()) {
    ...
    redirect(new moodle_url($baseurl, ['attempt' => $attemptid]),
        get_string('flagupdated', 'mod_authorcheck'), null,
...
if ($attemptid) {
    ...
    $attempt = $DB->get_record('authorcheck_attempts',
        ['id' => $attemptid, 'authorcheckid' => $instance->id], '*', MUST_EXIST);
    ...
    if (!empty($attempt->unprocessable)) {
        echo $OUTPUT->notification(get_string('unprocessablenote', 'mod_authorcheck'),
            \core\output\notification::NOTIFY_WARNING);
    } else if (!empty($attempt->nosubmission)) {
        echo $OUTPUT->notification(get_string('nosubmissionnote', 'mod_authorcheck'),
            \core\output\notification::NOTIFY_WARNING);
    }
    ...
    if (!empty($studentids)) {
        ...
        $attempts = $DB->get_records_select('authorcheck_attempts',
            "authorcheckid = :acid AND userid $insql", $params,
            'flagged DESC, timemodified DESC');
    }

    if (empty($attempts)) {
        echo $OUTPUT->notification(get_string('nosubmissions', 'mod_authorcheck'),
        ...
        foreach ($attempts as $a) {
            ...
            $flag = $a->flagged
                ? html_writer::tag('span', get_string('flagged', 'mod_authorcheck'),
